...
let dateDebArr = JSON.parse(divDD.textContent)
var dateDeb = new Date(dateDebArr[dateSelected]);

var divDF = document.getElementById("df");
let dateFinArr = JSON.parse(divDF.textContent)
let dateFin = new Date(dateFinArr[dateSelected]);
...
//x.ticks(d3.timeSecond.every(
    //numberOfPoints/numberOfSec));

if((numberOfSec/numberOfPoints)>=60){
    allDates = x.ticks(d3.timeMinute.every(
        (numberOfSec/numberOfPoints)/60));
}else{
    allDates = x.ticks(d3.timeSecond.every(numberOfSec/numberOfPoints));
}
